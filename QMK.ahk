...
ShellHookGui := Gui()
DllCall("RegisterShellHookWindow", "Ptr", ShellHookGui.Hwnd)
OnMessage(DllCall("RegisterWindowMessage", "Str", "SHELLHOOK"), ShellMessage)

ShellMessage(wParam, lParam, *) {
    if (wParam == 4 || wParam == 32772) ; HSHELL_WINDOWACTIVATED | HSHELL_RUDEAPPACTIVATED
        SetTimer(CheckFullscreen, -100) ; Check 100ms after switch to let window dimensions settle
}

; Configure the WASD-IJKL to be the modifiers
QMK.SetupModifier("a", "Ctrl")
QMK.SetupModifier("j", "Ctrl")

QMK.SetupModifier("s", "Shift")
QMK.SetupModifier("k", "Shift")

QMK.SetupModifier("d", "Win")
QMK.SetupModifier("l", "Win")

QMK.SetupModifier("w", "Alt")
QMK.SetupModifier("i", "Alt")
...
; additional window management - move to next screen (also use 'ds + h or l')
...
QMK.SetupCombo("h", "a", (*) => SendEvent("^{Left}"))
QMK.SetupCombo("h", "w", (*) => SendEvent("{Up}"))
QMK.SetupCombo("h", "s", (*) => SendEvent("{Down}"))
QMK.SetupCombo("h", "d", (*) => SendEvent("^{Right}"))
QMK.SetupInstantCombo("h", "q", (*) => SendEvent("{Backspace}"))
QMK.SetupInstantCombo("h", "e", (*) => SendEvent("{Delete}"))
QMK.SetupCombo("h", "u", (*) => Send("{Home}"))
QMK.SetupCombo("h", "i", (*) => Send("{End}"))

; g is another layer for moving, though bigger movements "Go" + ___. 
QMK.SetupInstantCombo("g", "q", (*) => SendEvent("^{Backspace}"))
QMK.SetupInstantCombo("g", "e", (*) => SendEvent("^{Delete}"))
QMK.SetupCombo("g", "s", (*) => Send("{Down}"))
QMK.SetupCombo("g", "w", (*) => Send("{Up}"))
QMK.SetupCombo("g", "u", (*) => Send("^{Home}")) ; go up
QMK.SetupCombo("g", "i", (*) => Send("^{End}")) 
...
; QMK.SetupCombo("w", "Space", (*) => SendEvent("w "))
...
; 'f' for "website" layer - "Website" + _____. Mostly including for ideas. Both require the UIA library, but other code is in
; QMK.SetupCombo("f", "c", (*) => chatgpt.screenshot())
; QMK.SetupCombo("f", "d", (*) => globals.activaterun("Google Docs", "https://docs.google.com/"))
; QMK.SetupCombo("f", "g", (*) => globals.activaterun("Gmail", "https://mail.google.com"))
; QMK.SetupCombo("f", "j", (*) => globals.activaterun("ChatGPT", "https://chatgpt.com/?temporary-chat=true&model=gpt-5-instant"))
; QMK.SetupCombo("f", "k", (*) => globals.activaterun("Claude", "https://claude.ai/new"))
; QMK.SetupCombo("f", "n", (*) => globals.activaterun("NotebookLM", "https://notebooklm.google.com/"))
; QMK.SetupCombo("f", "r", (*) => globals.activaterun("Grok", "https://grok.com"))
; QMK.SetupCombo("f", "p", (*) => globals.activaterun("Spotify for Creators", "https://creators.spotify.com/pod/show/7lNvdBsvWKhblGyVn80JTo/episodes?pageSize=30"))
; QMK.SetupCombo("f", "y", (*) => globals.activaterun("Youtube", "https://www.youtube.com"))
...
; QMK.SetupHold("j", ["global"], (*) => mm.GestureDL()) ; Disabled as j is now Ctrl
; QMK.SetupHold("k", ["global"], (*) => mm.GestureUR()) ; Disabled as k is now Shift
; QMK.SetupHold("l", ["global"], (*) => mm.SnapRight("A")) ; Disabled as l is now Win
...
CheckFullscreen() {
    static suspendedByScript := false
    isFullScreen := IsWindowFullScreen("A")
    
    if (isFullScreen) {
        if (!A_IsSuspended) {
            Suspend(true)
            suspendedByScript := true
        }
    } else {
        if (suspendedByScript && A_IsSuspended) {
            Suspend(false)
            suspendedByScript := false
        }
    }
}
...
IsWindowFullScreen(winTitle := "A") {
    try {
        if !winID := WinExist(winTitle)
            return false
        
        WinGetPos(&WinX, &WinY, &WinW, &WinH, winID)
        Loop MonitorGetCount() {
            MonitorGet(A_Index, &MonLeft, &MonTop, &MonRight, &MonBottom)
            if (WinX == MonLeft && WinY == MonTop && WinW == (MonRight - MonLeft) && WinH == (MonBottom - MonTop)) {
                return (WinGetClass(winID) != "Progman" && WinGetClass(winID) != "WorkerW")
            }
        }
    }
    return false
